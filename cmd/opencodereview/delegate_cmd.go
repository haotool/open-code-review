...
func runDelegate(args []string) error {
	return runDelegateWithIO(os.Stdout, os.Stderr, args)
}

func runDelegateWithIO(out, errOut io.Writer, args []string) error {
	if len(args) == 0 {
		printDelegateUsageTo(out)
		return nil
	}
	...
	case "-h", "--help":
		...
		return nil
	case "preview":
		return delegatecli.RunPreview(out, args[1:])
	case "rule":
		return delegatecli.RunRule(out, args[1:])
	default:
		fmt.Fprintf(errOut, "unknown delegate sub-command: %s\nRun 'ocr delegate -h' for usage\n", sub)
		return fmt.Errorf("unknown delegate sub-command: %s", sub)
	}
}

func printDelegateUsage() {
	printDelegateUsageTo(os.Stdout)
}

func printDelegateUsageTo(out io.Writer) {
	fmt.Fprintln(out, `OpenCodeReview - Delegation Mode
